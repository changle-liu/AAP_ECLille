...
#ifdef ELT_RPN
// T_elt est un éléments extraits d’une expression RPN ...
char * toString(T_elt e) {
	static char buffer[15]; 
	if(e.status == ENTIER){
		sprintf(buffer, "%d",e.value);
	}else if(e.status == NON_ENTIER){	
		sprintf(buffer, "non entier");
	}else if(e.status == INVALIDE){
		sprintf(buffer, "exp invalide");
	}else{	
		//si e est un operateur
		if(e.value == PLUS){
			sprintf(buffer, "+");
		}
		if(e.value == MOIN){
