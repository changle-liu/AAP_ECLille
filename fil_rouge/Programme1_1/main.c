...
int main(void) { 
	...
	*/
	// tri par tas V3
	T_data data_1 = {0, ex4};
	data_1 = heapSortV3(data_1, 10);
	T_heap * maximier = NULL;
	maximier = initHeap(data_1.pElt, 10);
	showHeap(maximier);
}
...
void heap_ajust(T_elt * arr, int start, int end) {  
    ...
    int son = iLCHILD(dad);  
    while (son <= end) { //   
        if (son + 1 <= end && eltCmp(arr[son], arr[son + 1]) < 0){ //先比较两個子节点大小，选择最大的  
            son++;  
            stats.nbComparisons++;
            }
        stats.nbComparisons++;
        if (eltCmp(arr[dad], arr[son]) > 0) //如果父节点大于子节点代表调整完毕，直接跳出函数  
            return;  
